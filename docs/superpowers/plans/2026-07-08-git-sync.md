...
>
> **Delete this file (and its spec) once implemented** — see "Cleanup once
> implemented" at the bottom. If you are reading this and every task below is
> checked off and shipped, this file is overdue for removal.
...
## Cleanup once implemented

Repo convention: specs/plans live under `docs/superpowers/` only until the
work ships (`docs/gitsync.md` is the shipped doc). After final verification
passes, remove this plan and its spec, and re-point the spec link in
`docs/future.md` (the "Per-project git sync" entry) at `docs/gitsync.md`:

```bash
git rm docs/superpowers/plans/2026-07-08-git-sync.md \
       docs/superpowers/specs/2026-07-08-git-sync-design.md
# then edit docs/future.md: link gitsync.md instead of the removed spec
git commit -m "Remove shipped git-sync spec/plan; docs/gitsync.md is the doc of record"
```
