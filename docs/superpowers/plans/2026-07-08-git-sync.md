...
### Task 7: Docs and e2e smoke test
...
- Produces: user-facing docs; an e2e assertion that the controls render disabled when git sync is unconfigured (the default dev stack).
...
- [ ] **Step 3: Add the e2e smoke test**

Create `e2e/test_gitsync.py`:

```python
"""Git-sync controls render on the dashboard page and are disabled when git
sync is unconfigured (the default dev stack has no GIT_SYNC_REMOTE)."""

from playwright.sync_api import Page, expect


def test_dashboard_page_shows_disabled_git_controls(page: Page, base_url: str):
...
- [ ] **Step 4: Run the e2e test (requires the dev stack)**

Start the dev servers (`npm run dev` from the repo root, per README), then:
Run: `uv run --group test pytest e2e/test_gitsync.py -v`
Expected: PASS. If the dev stack cannot be started in this environment, verify instead with `npm run build -w frontend` and the backend suite, and say so explicitly in the task report.

- [ ] **Step 5: Commit**

```bash
git add docs/gitsync.md docs/api.md e2e/test_gitsync.py
git commit -m "Document git sync and add e2e smoke test for the controls"
```
