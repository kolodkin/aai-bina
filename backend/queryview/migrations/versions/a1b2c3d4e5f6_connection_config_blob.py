...
down_revision: str | None = "9a536b7c0328"
...
def upgrade() -> None:
    ...
    conn = op.get_bind()
    rows = conn.execute(sa.text("SELECT id, host, port, username, password FROM connections")).fetchall()
    for rid, host, port, username, password in rows:
        plain = _decrypt_str(password) if password else ""
        blob = _encrypt_str(json.dumps({"host": host, "port": port, "username": username, "password": plain}))
        conn.execute(
